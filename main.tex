...
%The homomorphism $\Phi$ is surjective so $\Spec(R)$ is a closed subscheme of $\Spec(S)$ cut out by the ideal $I:= \ker \Phi$.
...
% The definition of families of refined representations Definition \ref{def:refined} does not use the fact that the bundle $\oplus T_i$ is tilting.
...
% Following Craw-Smith theory of multi-linear series \cite{Craw-Smith}, given a projective DM stack $\bX$ with a finite subset $Q_0$ of $\Pic(\bX)$ we study the moduli space of refined representations $\bM_\text{ref}$ and the corresponding morphism $\bX \rightarrow \bM_\text{ref}$ in what follows.
...
% $$ A = \End_{R-\Gr} \bigoplus_{\lambda \in Q_0} R(\lambda).$$
% As usual, there is a quiver $Q = (Q_0,Q_1)$ such that $A$ is naturally given by $kQ/I$.
% For $\bM$, the rigidified moduli stack of $A$-modules of dimension vector $\vec{1}$, we have a universal morphism $f \colon \bX \rightarrow \bM$.

% The moduli space $\bM$ may be expressed as global quotient as follows.


% Let $S$ denote the symmetric algebra generated by the sets $A_{\lambda\mu}$ for every $(\lambda, \mu) \in Q_0^2$; this ring is canonically isomorphic to $k[y_a \,|\, a \in Q_1]$.
% The ring $S$ is naturally $\bZ^{Q_0}$ graded https://translate.google.com/#view=home&op=translate&sl=auto&tl=en&text=gestione%20affidatawith $\deg(y_a) = - \chi_{t(a)} + \chi_{h(a)}$.
% Furthermore, $S$ comes with a universal homomorphism of $\bZ^{Q_0}$-graded rings to $R$ taking $y_a$ to its corresponding section in $\Hom(R(t(a)),\, R(h(a)))$.
% The final component of the ideal 
% $$I_Q := \langle y_p - y_{p'} \, | \, p,p' \in A_{\lambda\mu}, $$
